...
export default function CreateSalatModule({ handleCloseModule }) {
  ...
  const postProduct = async () => {
    try {
      let response = await axios.post("http://localhost:3000/salats", {
        img,
        title,
        price,
      });
  ...
  return (
    <React.Fragment>
      <div className="w-[450px] bg-white p-7 shadow-xl rounded-3xl flex flex-col items-center gap-y-5 relative">
        <p className="text-ba text-xl font-normal">Добавить новый салат</p>
        <input
          type="text"
