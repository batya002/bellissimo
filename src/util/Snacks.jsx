...
import CreateModule from "../components/modules/CreateSnackModule";
...
export default function Snacks() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:3000/snacks/${id}`);
  ...
  const handlerEdit = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3000/snacks/${selectedProduct.id}`,
        {
    ...
    <React.Fragment>
      <section className="my-[40px]">
        <div className="container w-[1180px]">
          <h2 className="text-[48px] text-center font-normal mb-5">Закуски</h2>
