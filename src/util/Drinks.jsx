...
import CreateModule from "../components/modules/CreateDrinkModule";
...
export default function Drinks() {
  ...
  const getData = async () => {
    try {
      let response = await axios.get("http://localhost:3000/drinks");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:3000/drinks/${id}`);
  ...
  const handlerEdit = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3000/drinks/${selectedProduct.id}`,
        {
    ...
    <React.Fragment>
      <section className="my-[40px]">
        <div className="container w-[1180px]">
          <h2 className="text-[48px] text-center font-normal mb-5">Напитки</h2>
          <ul className="flex flex-wrap justify-between gap-y-5">
            {data.map((value, index) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="text-xl font-bold mb-[16px] truncate">{title}</h3>
            ...
            >
              ...
              <p className="text-ba text-2xl">Создать напиток</p>
            </li>
            {isModuleOpen && (
