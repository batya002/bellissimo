...
import CreateModule from "../components/modules/CreatePizzaModule";
...
export default function Pizza() {
  ...
  const getData = async () => {
    try {
      let response = await axios.get("http://localhost:3000/pizzas");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:3000/pizzas/${id}`);
  ...
  const handlerEdit = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3000/pizzas/${selectedProduct.id}`,
        {
    ...
    <React.Fragment>
      <section className="my-[40px]">
        <div className="container w-[1180px]">
          <h2 className="text-[48px] text-center font-normal mb-5">Пицца</h2>
          <ul className="flex flex-wrap justify-between gap-y-5">
            ...
            >
              ...
              <p className="text-ba text-3xl">Создать комбо</p>
            </li>
            {isModuleOpen && (
        ...
        >
          ...
          >
            ...
            <form>
              ...
              <div className="mb-4">
                <label className="block text-sm font-semibold">Описание</label>
                <textarea
