...
import CreateModule from "../components/modules/CreateSalatModule";
...
export default function Salats() {
  ...
  const getData = async () => {
    try {
      let response = await axios.get("http://localhost:3000/salats");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:3000/salats/${id}`);
  ...
  const handlerEdit = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3000/salats/${selectedProduct.id}`,
        {
    ...
    <React.Fragment>
      <section className="my-[80.7px]">
        <div className="container w-[1180px]">
          <h2 className="text-[48px] text-center font-normal mb-5">Салаты</h2>
          <ul className="flex flex-wrap justify-between gap-y-5">
            ...
            >
              ...
              <p className="text-ba text-2xl">Создать салат</p>
            </li>
            {isModuleOpen && (
