...
import CreateModule from "../components/modules/CreateSouceModule";
...
export default function Souces() {
  ...
  const getData = async () => {
    try {
      let response = await axios.get("http://localhost:3000/souces");
  ...
  const handlerEdit = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3000/souces/${selectedProduct.id}`,
        {
    ...
    <React.Fragment>
      <section className="my-[80.7px]">
        <div className="container w-[1180px]">
          <h2 className="text-[48px] text-center font-normal mb-5">Соусы</h2>
          <ul className="flex flex-wrap justify-between gap-y-5">
            ...
            >
              ...
              <p className="text-ba text-2xl">Создать соус</p>
            </li>
            {isModuleOpen && (
