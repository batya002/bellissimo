...
import CreateModule from "../components/modules/CreateComboModule";
...
export default function Combo() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:3000/combos/${id}`);
  ...
  const handlerEdit = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3000/combos/${selectedProduct.id}`,
        {
    ...
    <React.Fragment>
      <section className="my-[40px]">
        <div className="container w-[1180px]">
          <h2 className="text-[48px] text-center font-normal mb-5">Комбо</h2>
